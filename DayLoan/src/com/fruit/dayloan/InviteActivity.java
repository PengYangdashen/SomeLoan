...
public class InviteActivity extends BaseActivity implements OnClickListener {
	...
	private void initView() {
		...
		userId = sp.getString("userid", "");
		 ...
		 HttpUtils.doGetAsyn(com.fruit.dayloan.utils.Config.HOME_INIT + "&userid=" + userId, mHandler, com.fruit.dayloan.utils.Config.CODE_HOME_INIT);
	...
	private Handler mHandler = new Handler() {
		public void handleMessage(android.os.Message msg) {
			switch (msg.what) {
			case com.fruit.dayloan.utils.Config.CODE_HOME_INIT:
...
//							.getString("rzstatus"));
					...
					if (rzname.equals("")) {
						url = com.fruit.dayloan.utils.Config.SHARE_CORD+"tgid="+userId+"&yqren="+mobilephone;
					}else {
						try {
							url = com.fruit.dayloan.utils.Config.SHARE_CORD+"tgid="+userId+"&yqren="+URLEncoder.encode(rzname, "UTF-8");
				...
				break;
			case com.fruit.dayloan.utils.Config.CODE_URL_ERROR:
				Toast.makeText(InviteActivity.this, "url错误", Toast.LENGTH_SHORT)
						.show();
				break;
			case com.fruit.dayloan.utils.Config.CODE_NET_ERROR:
				Toast.makeText(InviteActivity.this, "网络错误", Toast.LENGTH_SHORT)
						.show();
				break;
			case com.fruit.dayloan.utils.Config.CODE_SHARE:
	...
	private void showShare() {
		 ...
		 oks.setTitle("送你个借款神器，人人闪贷");
		 // titleUrl是标题的网络链接，仅在Linked-in,QQ和QQ空间使用
		 oks.setTitleUrl(url);
		 // text是分享文本，所有平台都需要这个字段
		 oks.setText("有身份证就能轻松借5000元，纯信用，秒到账......");
		 //分享网络图片，新浪微博分享网络图片需要通过审核后申请高级写入接口，否则请注释掉测试新浪微博
		 oks.setImageUrl(com.fruit.dayloan.utils.Config.SHARELOGO_CODE);
		 // imagePath是图片的本地路径，Linked-In以外的平台都支持此参数
		 //oks.setImagePath("/sdcard/test.jpg");//确保SDcard下面存在此张图片
		 // url仅在微信（包括好友和朋友圈）中使用
		 oks.setUrl(url);
		 // comment是我对这条分享的评论，仅在人人网和QQ空间使用
		 oks.setComment("没钱就找人人闪贷");
