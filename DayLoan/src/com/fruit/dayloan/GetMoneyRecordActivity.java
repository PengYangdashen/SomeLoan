...
import com.fruit.dayloan.adapter.GetMoneyRecordAdapter;
...
public class GetMoneyRecordActivity extends BaseActivity implements
	...
	private void initView() {
		...
		adapter = new GetMoneyRecordAdapter(GetMoneyRecordActivity.this,
				arrayList);
		getmoney_list.setAdapter(adapter);

		HttpUtils.doGetAsyn(Config.TXMONEY_CORD + "&userid=" + userId
				+ "&curPage=" + curPage, mHandler, Config.CODE_TXMONEY);
	...
	private Handler mHandler = new Handler() {
		public void handleMessage(android.os.Message msg) {
			...
			case Config.CODE_URL_ERROR:
				Toast.makeText(GetMoneyRecordActivity.this, "url错误",
						Toast.LENGTH_SHORT).show();
				break;
			case Config.CODE_NET_ERROR:
				Toast.makeText(GetMoneyRecordActivity.this, "网络错误",
						Toast.LENGTH_SHORT).show();
				break;
			...
			case Config.CODE_TXMONEY:
				...
				try {
					...
					get_money_value.setText(json.getString("presentationTotal")
							+ "元");
					...
					if (array.length() == 0) {
						...
						for (int i = 0; i < array.length(); i++) {
							...
							int type = object.optInt("status");
							JSONObject time = null;
							if (type == 1) {// applytime
								time = object.getJSONObject("applytime");
								pojo.setTime(TimeUtils.parseDate2(Long
	...
	private void setDialog() {
		new com.fruit.dayloan.utils.AlertDialog(GetMoneyRecordActivity.this)
				.builder().setMsg("您还没有提现记录")
