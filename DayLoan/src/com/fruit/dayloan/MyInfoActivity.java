...
import com.fruit.dayloan.MyInfoActivity;
...
public class MyInfoActivity extends BaseActivity implements OnClickListener,
	...
	private void initView() {
		...
		LinearLayout emial_linear = (LinearLayout) findViewById(R.id.emial_linear);
		emial_linear.setOnClickListener(this);
                 ...
                 if (moxieCallBackData != null) {
                     ...
                     switch (moxieCallBackData.getCode()) {
                         ...
                         case MxParam.ResultCode.THIRD_PARTY_SERVER_ERROR:
                             Toast.makeText(MyInfoActivity.this, "导入失败(平台方服务问题)", Toast.LENGTH_SHORT).show();
                             break;
                         case MxParam.ResultCode.MOXIE_SERVER_ERROR:
                             Toast.makeText(MyInfoActivity.this, "导入失败(魔蝎数据服务异常)", Toast.LENGTH_SHORT).show();
                             break;
                         case MxParam.ResultCode.USER_INPUT_ERROR:
                             Toast.makeText(MyInfoActivity.this, "导入失败(" + moxieCallBackData.getMessage() + ")", Toast.LENGTH_SHORT).show();
                             break;
                         case MxParam.ResultCode.IMPORT_FAIL:
                             Toast.makeText(MyInfoActivity.this, "导入失败", Toast.LENGTH_SHORT).show();
                             break;
                         case MxParam.ResultCode.IMPORT_SUCCESS:
                             ...
                             switch (moxieCallBackData.getTaskType()) {
                                 ...
                                 case MxParam.PARAM_FUNCTION_CARRIER:
                                     Toast.makeText(MyInfoActivity.this, "运营商导入成功", Toast.LENGTH_SHORT).show();
                                  // 运营商导入
          							HttpUtils.doGetAsyn(Config.AUTHMOBILE_CORD
          									+ "&userid=" + userId + "&code=" + moxieCallBackData.getCode()
          									+ "&taskId=" + moxieCallBackData.getTaskId() + "&outOrderId="
          									+ "", mHandler,
          									Config.CODE_AUTHMOBILE);
                                     break;
                                 case MxParam.PARAM_FUNCTION_TAOBAO:
                                     Toast.makeText(MyInfoActivity.this, "淘宝导入成功", Toast.LENGTH_SHORT).show();
                                 ...
                                 case MxParam.PARAM_FUNCTION_JINGDONG:
                                	 Toast.makeText(MyInfoActivity.this, "京东导入成功", Toast.LENGTH_SHORT).show();
                                	 HttpUtils.doGetAsyn(Config.AUTHJD_CORD + "&userid="
          									+ userId + "&code=" + moxieCallBackData.getCode() + "&taskId="
          									+ moxieCallBackData.getTaskId() + "&outOrderId=" + "",
          									mHandler, Config.CODE_AUTHMOBILE);
                                	 break;
                                 //.....
                                 default:
                                     Toast.makeText(MyInfoActivity.this, "导入成功", Toast.LENGTH_SHORT).show();
                             }
                             moxieContext.finish();
                             return true;
                     }
