...
public class CameraActivity extends BaseActivity {
	...
	private TextView jk_referne,jk_lx,jk_bank;
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		jk_lx = (TextView)findViewById(R.id.jk_lx);
		jk_bank = (TextView)findViewById(R.id.jk_bank);
		...
		confirm.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				new AlertDialog(CameraActivity.this).builder().setMsg("拍摄视频时请说出下列话：我叫"+realname+"通过粒粒贷成功借款"+pf_money.getText().toString())
...
//				finish();
			...
			case Config.CODE_BACKMONEYINIT:
				...
				try {
					...
					jk_bank.setText(cardno.substring(0,3)+"**********"+cardno.substring((cardno.length())-4,cardno.length()));
					...
					if (jk_dates==1) {
						...
						low = (all*0.03*0.5);
						fw = (all*0.27*0.5);
						jk_lx.setText(df.format(all*0.3*0.5)+"元");  //总费用
						jk_referne.setText(df.format(all*0.3*0.5)+"元");  //总费用
					}else {
						jk_date.setText("30天");
						low = (all*0.03);
						fw = (all*0.27);
						jk_lx.setText(df.format(all*0.3)+"元");  //总费用
						jk_referne.setText(df.format(all*0.3)+"元");  //总费用
					}
					low_referne.setText(df.format(low)+"元");
					fx_money.setText(df.format(fw)+"元");
...
//							.show();
					final String fileName = "lld" + userId + "=" + System.currentTimeMillis();
